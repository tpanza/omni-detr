...
def erase(image, target, region):
    ...
    h = h / img_h
    w = w / img_w
    keep = ~((boxes_xy[:, 0] > j) & (boxes_xy[:, 1] > i) &
             (boxes_xy[:, 2] < j + w) & (boxes_xy[:, 3] < i + h))
    for field in fields:
...
class RandomSizeCrop(object):
    ...
    def __call__(self, img: PIL.Image.Image, target: dict, record: dict):
        ...
        record[
            'RandomSizeCrop'] = region  # i,j,h,w, i,j are not the center, look like is the left top coordinate
...
class RandomErasing1(object):
    ...
    def __call__(self, img, target, record):
        if random.random() < 0.7:
            region = T.RandomErasing.get_params(img,
                                                scale=(0.05, 0.2),
                                                ratio=(0.3, 3.3))
