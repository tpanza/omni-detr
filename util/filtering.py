...
def box_calibration(cur_boxes, cur_labels, cur_idx, records_unlabel_q,
    ...
    if 'RandomFlip' in records_unlabel_k.keys(
    ) and records_unlabel_k['RandomFlip']:
    ...
    if 'RandomFlip' in records_unlabel_q.keys(
    ) and records_unlabel_q['RandomFlip']:
        cur_boxes = cur_boxes[:, [2, 1, 0, 3]] * torch.as_tensor(
            [-1, 1, -1, 1]).cuda() + torch.as_tensor([img_w, 0, img_w, 0
                                                      ]).cuda()
    ...
    if records_unlabel_q['RandomResize_times'] > 1:
        ...
        rescaled_size1 = get_size_with_aspect_ratio((img_w, img_h),
                                                    rescaled_size1)
        ratios = tuple(
            float(s) / float(s_orig)
            for s, s_orig in zip(rescaled_size1, (img_w, img_h)))
        ratio_width, ratio_height = ratios
        cur_boxes = cur_boxes * torch.as_tensor(
            [ratio_width, ratio_height, ratio_width, ratio_height]).cuda()
        ...
        rescaled_size2 = get_size_with_aspect_ratio((img_w, img_h),
                                                    rescaled_size2,
                                                    max_size=max_pixels)
        ratios = tuple(
            float(s) / float(s_orig)
            for s, s_orig in zip(rescaled_size2, (img_w, img_h)))
        ratio_width, ratio_height = ratios
        cur_boxes = cur_boxes * torch.as_tensor(
            [ratio_width, ratio_height, ratio_width, ratio_height]).cuda()
    ...
    cur_boxes = cur_boxes / torch.tensor([img_w, img_h, img_w, img_h],
    ...
    if 'RandomErasing1' in records_unlabel_q.keys():
        ...
        h = h / img_h
        w = w / img_w
        keep = ~((cur_boxes_xy[:, 0] > j) & (cur_boxes_xy[:, 1] > i) &
                 (cur_boxes_xy[:, 2] < j + w) & (cur_boxes_xy[:, 3] < i + h))
        cur_boxes = cur_boxes[keep]
        cur_labels = cur_labels[keep]
        cur_idx = cur_idx[keep]
...
def unified_filter_pseudo_labels(pseudo_unsup_outputs,
                                 targets_unlabel_q,
                                 targets_unlabel_k,
                                 records_unlabel_q,
                                 records_unlabel_k,
    ...
    if is_binary:
        satisfied_idx_d2 = (pseudo_unsup_prob[0, :, 0] > threshold).nonzero(
            as_tuple=True)
        satisfied_idx_d1 = torch.zeros(satisfied_idx_d2[0].shape[0],
        ...
        satisfied_bbox = pseudo_unsup_bbox[satisfied_idx[0],
                                           satisfied_idx[1], :]
        satisfied_class = torch.zeros(satisfied_bbox.shape[0],
                                      dtype=torch.long).cuda()
    else:
        if label_type == 'Unsup':
            classes_scores, classes_indices = torch.max(pseudo_unsup_prob,
                                                        dim=2)
            satisfied_idx = (classes_scores > threshold).nonzero(as_tuple=True)
            pseudo_unsup_bbox = pseudo_unsup_outputs['pred_boxes']
            satisfied_bbox = pseudo_unsup_bbox[satisfied_idx[0],
                                               satisfied_idx[1], :]
            satisfied_class = classes_indices[satisfied_idx[0],
                                              satisfied_idx[1]]
            ...
            for i_c in targets_gt_unlabel_q_list:
                ...
                satisfied_idx_i = (classes_scores_i > threshold).nonzero(
                    as_tuple=True)
                if len(satisfied_idx_i[0]) > 0:
                    targets_gt_unlabel_q_new = torch.cat(
                        (targets_gt_unlabel_q_new, i_c * torch.ones(
                            len(satisfied_idx_i[0]), dtype=torch.long).cuda()))
                else:
                    targets_gt_unlabel_q_new = torch.cat(
                        (targets_gt_unlabel_q_new,
                         i_c * torch.ones(1, dtype=torch.long).cuda()))
            ...
            dist_bbox_tags = torch.zeros(
                (pseudo_unsup_prob.shape[1],
                 targets_gt_unlabel_q.shape[0])).cuda()
            for i in range(targets_gt_unlabel_q.shape[0]):
                dist_bbox_tags[:, i] = 1 - pseudo_unsup_prob[
                    0, :, targets_gt_unlabel_q[i]]
            dist_bbox_tags = dist_bbox_tags.cpu()

            pseudo_unsup_bbox = pseudo_unsup_outputs['pred_boxes']
            indices = scipy.optimize.linear_sum_assignment(dist_bbox_tags)
            updated_idx_d1 = torch.zeros(targets_gt_unlabel_q.shape[0],
                                         dtype=torch.long).cuda()
            updated_idx_d2_1 = indices[0].tolist()
            updated_idx_d2_2 = indices[1].tolist()
            updated_idx_d2 = [
                x for _, x in sorted(zip(updated_idx_d2_2, updated_idx_d2_1))
            ]
            updated_idx_d2 = torch.tensor(updated_idx_d2,
                                          dtype=torch.long).cuda()
            ...
            indicator_include_matrix = torch.zeros(
                (pseudo_unsup_prob.shape[1],
                 targets_gt_point_unlabel_q.shape[0]),
                dtype=torch.long).cuda()
            for i in range(targets_gt_point_unlabel_q.shape[0]):
                ...
                y0 = point_i[1]
                keep_i = (
                    pseudo_unsup_bbox[0, :, 0] - pseudo_unsup_bbox[0, :, 2] / 2
                    < x0) & (pseudo_unsup_bbox[0, :, 0] +
                             pseudo_unsup_bbox[0, :, 2] / 2 >
                             x0) & (pseudo_unsup_bbox[0, :, 1] -
                                    pseudo_unsup_bbox[0, :, 3] / 2 <
                                    y0) & (pseudo_unsup_bbox[0, :, 1] +
                                           pseudo_unsup_bbox[0, :, 3] / 2 > y0)
                keep_i = keep_i.int()
                keep_i = keep_i.type(torch.cuda.LongTensor)
                if keep_i.sum() > 0:
                    indicator_include_matrix[keep_i == 1, i] = 1
            indicator_include_matrix[
                indicator_include_matrix <
                1] = 1e3  # just give a random large number

            # Compute the distance cost between boxes
            dist_bbox_point = torch.cdist(pseudo_unsup_bbox[0][:, :2],
                                          targets_gt_point_unlabel_q[:, :2],
                                          p=2)
            ...
            dist_bbox_point = (dist_bbox_point +
                               matrix_w_high_score) * indicator_include_matrix
            ...
            cost_bbox = torch.cdist(pseudo_unsup_bbox[0],
                                    targets_gt_box_unlabel_q,
                                    p=1)

            # Compute the giou cost betwen boxes
            cost_giou = -generalized_box_iou(
                box_cxcywh_to_xyxy(pseudo_unsup_bbox[0]),
            ...
            updated_idx_d1 = torch.zeros(targets_gt_box_unlabel_q.shape[0],
                                         dtype=torch.long).cuda()
            updated_idx_d2_1 = indices[0].tolist()
            updated_idx_d2_2 = indices[1].tolist()
            updated_idx_d2 = [
    ...
    for i in range(pseudo_unsup_bbox.shape[0]):
        ...
        if len(
                i_idx[0]
        ) == 0:  # means no predicted bbox satisfies the confidence threshold
            # just assign []
            for key, value in i_target.items():
                if key == 'boxes':
                    i_dict['boxes'] = torch.empty((0),
                                                  dtype=torch.int64,
                                                  device='cuda')
                elif key == 'labels':
                    i_dict['labels'] = torch.empty((0),
                                                   dtype=torch.int64,
                                                   device='cuda')
                else:
            ...
            cur_i_idx = i_idx[0]
            i_boxes, i_labels, cur_i_idx = box_calibration(
                i_boxes, i_labels, cur_i_idx, records_unlabel_q,
                records_unlabel_k, pixels)
            for key, value in i_target.items():
                if key == 'boxes':
