...
class CocoDetection_semi(TvCocoDetection_semi):
    ...
    def __init__(self,
                 ...
                 local_rank=0,
                 local_size=1):
        super(CocoDetection_semi, self).__init__(img_folder,
                                                 ann_file,
                                                 cache_mode=cache_mode,
                                                 local_rank=local_rank,
                                                 local_size=local_size)
    ...
    def __getitem__(self, idx):
        img, target, indicator, labeltype = super(CocoDetection_semi,
        ...
        if self._transforms_strong is not None:
            ...
            img_q, target_q, record_q = self._transforms_strong(
                img, target, record_q)
        if self._transforms_weak is not None:
            record_k = {}
            record_k['OriginalImageSize'] = [img.height, img.width]
            img_k, target_k, record_k = self._transforms_weak(
                img, target, record_k)
...
def build(image_set, args):
    ...
    if args.dataset_file == "voc_omni":
        PATHS = {
            "train":
            (root / "VOCdevkit/VOC20072012trainval/JPEGImages", root /
             "VOCdevkit/VOC20072012trainval" / args.annotation_json_label),
            "val":
            (root / "VOCdevkit/VOC2007test/JPEGImages", root /
             "VOCdevkit/VOC2007test" / 'instances_VOC_test2007_test.json'),
        }
    elif args.dataset_file == "voc_semi":
        PATHS = {
            "train":
            (root / "VOCdevkit/VOC2007trainval/JPEGImages",
        ...
        PATHS = {
            "train":
            (root / "ML-Data_cleared", root / args.annotation_json_label),
            "val":
            (root / "ML-Data_cleared", root / 'instances_bees_semi_val.json'),
        }
        ...
        PATHS = {
            "train": (root / "train_objects365",
                      root / "annotations" / args.annotation_json_label),
            "val": (root / "val_objects365",
                    root / "annotations" / 'objects365_val_w_indicator.json'),
        }
    else:
        PATHS = {
            ...
            "val":
            (root / "val2017",
             root / "annotations" / 'instances_w_indicator_val2017.json'),
        }
    ...
    if args.dataset_file == "voc_semi":
        ...
        PATHS = {
            ...
            (root / "VOCdevkit/VOC20072012trainval/JPEGImages", root /
             "VOCdevkit/VOC20072012trainval" / args.annotation_json_label),
            "val":
            (root / "val2014", root / "annotations" / f'{mode}_val2014.json'),
        }
        ...
        PATHS = {
            "train": (root / "train2017",
                      root / "annotations" / args.annotation_json_label),
            "val": (root / "val2017",
                    root / "annotations" / f'{mode}_w_point_val2017.json'),
        }

    img_folder, ann_file = PATHS[image_set]
    if args.pixels == 600:
        dataset = CocoDetection_semi(
            img_folder,
            ann_file,
            transforms_strong=make_coco_strong_transforms_with_record600(
                image_set),
            transforms_weak=make_coco_weak_transforms_with_record600(
                image_set),
            return_masks=args.masks,
            cache_mode=args.cache_mode,
            local_rank=get_local_rank(),
        ...
        dataset = CocoDetection_semi(
            img_folder,
            ann_file,
            transforms_strong=make_coco_strong_transforms_with_record(
                image_set),
...
def build_semi_unlabel(image_set, args):
    ...
    if args.dataset_file == "voc_semi":
        PATHS = {
            "train":
            (root / "VOCdevkit/VOC2012trainval/JPEGImages", root /
             "VOCdevkit/VOC2012trainval" / args.annotation_json_unlabel),
            "val":
            (root / "OCdevkit/VOC2007test/JPEGImages",
             root / "VOCdevkit/VOC2007test" / 'instances_VOC_test2007.json'),
        }
        ...
        PATHS = {
            "train": (root / "train2014",
                      root / "annotations" / args.annotation_json_unlabel),
            "val":
            (root / "val2014", root / "annotations" / f'{mode}_val2014.json'),
        }
    elif args.dataset_file == "voc_omni":
        PATHS = {
            "train":
            (root / "VOCdevkit/VOC20072012trainval/JPEGImages", root /
             "VOCdevkit/VOC20072012trainval" / args.annotation_json_unlabel),
            "val":
            (root / "val2014", root / "annotations" / f'{mode}_val2014.json'),
        }
    elif args.dataset_file == "coco_add_semi":
        PATHS = {
            "train": (root / "unlabeled2017",
                      root / "annotations" / args.annotation_json_unlabel),
            "val": (root / "val2017",
                    root / "annotations" / f'{mode}_w_point_val2017.json'),
        }
        ...
        PATHS = {
            "train":
            (root / "ML-Data_cleared", root / args.annotation_json_unlabel),
            "val":
            (root / "ML-Data_cleared", root / 'instances_bees_semi_val.json'),
        }
