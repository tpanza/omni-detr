...
class SetCriterion_semi(nn.Module):
    ...
    def loss_labels(self,
        ...
        loss_ce = sigmoid_weighted_focal_loss(src_logits,
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, indicators,
                 **kwargs):
        loss_map = {
            'labels': self.loss_labels,
            'cardinality': self.loss_cardinality,
        ...
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, indicators,
                              **kwargs)
...
def build_semi(args):
    ...
    if args.masks:
        model_student = DETRsegm(model_student,
    ...
    if args.masks:
        model_teacher = DETRsegm(model_teacher,
                                 freeze_detr=(args.frozen_weights is not None))
    ...
    if args.masks:
        losses += ["masks"]
    # num_classes, matcher, weight_dict, losses, focal_alpha=0.25
    criterion = SetCriterion_semi(num_classes,
                                  matcher,
                                  weight_dict,
                                  losses,
                                  focal_alpha=args.focal_alpha)
    criterion.to(device)
    criterion_burnin = SetCriterion(num_classes,
                                    matcher,
                                    weight_dict,
                                    losses,
                                    focal_alpha=args.focal_alpha)
