...
def train_one_epoch_semi(model_student: torch.nn.Module,
                         model_teacher: torch.nn.Module,
                         criterion: torch.nn.Module,
                         data_loader_label: Iterable,
                         data_loader_unlabel: Iterable,
                         optimizer: torch.optim.Optimizer,
                         device: torch.device,
                         epoch: int,
                         max_norm: float = 0,
                         threshold: float = 0.7,
                         ema_keep_rate: float = 0.9996,
                         passed_num_path: str = '',
                         train_setting: str = '',
                         pixels: int = 800,
                         w_p: float = 0.5,
    ...
    prefetcher_label = data_prefetcher_semi(data_loader_label,
                                            device,
                                            prefetch=True)
    prefetcher_unlabel = data_prefetcher_semi(data_loader_unlabel,
                                              device,
                                              prefetch=True)
    ...
    # for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
    for _ in metric_logger.log_every(range(len(data_loader_label)), print_freq,
                                     header):
        ...
        if train_setting == 'coco_add_semi' or train_setting == 'voc_semi':
            pseudo_label_k = unified_filter_pseudo_labels(outputs_unsup_q,
                                                          targets_unlabel_q,
                                                          targets_unlabel_k,
                                                          records_unlabel_q,
                                                          records_unlabel_k,
        ...
        if train_setting == 'coco_omni' or train_setting == 'bees_omni' or train_setting == 'voc_omni' or train_setting == 'objects_omni' or train_setting == 'crowdhuman_omni':
            if labeltype_unlabel[0] == 'tagsU':
                pseudo_label_k = unified_filter_pseudo_labels(
                    outputs_unsup_q,
                    targets_unlabel_q,
                    targets_unlabel_k,
                    records_unlabel_q,
                ...
                pseudo_label_k = unified_filter_pseudo_labels(
                    ...
                    is_binary=False,
                    threshold=threshold)
            elif labeltype_unlabel[0] == 'boxesEC' or labeltype_unlabel[
                    0] == 'boxesU':
                pseudo_label_k = unified_filter_pseudo_labels(
                    outputs_unsup_q,
                    targets_unlabel_q,
        ...
        if max_norm > 0:
            grad_total_norm = torch.nn.utils.clip_grad_norm_(
                model_student.parameters(), max_norm)
        else:
            grad_total_norm = utils.get_total_grad_norm(
                model_student.parameters(), max_norm)
        optimizer.step()
...
def train_one_epoch_burnin(model: torch.nn.Module,
                           criterion: torch.nn.Module,
                           data_loader: Iterable,
                           optimizer: torch.optim.Optimizer,
                           device: torch.device,
