...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument(
        '--dataset_file', default='coco_omni'
    )  # coco_omni, coco_35to80_tagsU, coco_35to80_point, coco_objects_tagsU, coco_objects_points, bees_omni, voc_semi_ voc_omni, objects_omni, crowdhuman_omni
    ...
    parser.add_argument('--BURN_IN_STEP',
                        default=20,
                        type=int,
                        help='as the name means')
    parser.add_argument('--TEACHER_UPDATE_ITER',
                        default=1,
                        type=int,
                        help='as the name means')
    parser.add_argument('--EMA_KEEP_RATE',
                        default=0.9996,
                        type=float,
                        help='as the name means')
    parser.add_argument('--annotation_json_label',
                        default='',
                        help='percent with fully labeled')
    parser.add_argument('--annotation_json_unlabel',
                        default='',
                        help='percent with fully labeled')
    ...
    parser.add_argument('--save_freq',
                        ...
                        type=int,
                        metavar='N',
                        help='checkpoint save frequency (default: 5)')
...
def main(args):
    ...
    if args.dataset_file == "coco":
        ...
        model_student, model_teacher, criterion, criterion_burnin, postprocessors = build_model_semi(
            args)
    ...
    n_parameters = sum(p.numel() for p in model_student.parameters()
                       if p.requires_grad)
    print('number of student model params:', n_parameters)

    n_parameters = sum(p.numel() for p in model_teacher.parameters()
                       if p.requires_grad)
    ...
    if args.dataset_file == "coco":
        ...
        dataset_train_label = build_dataset(image_set='train',
                                            label=True,
                                            args=args)
        dataset_train_unlabel = build_dataset(image_set='train',
                                              label=False,
                                              args=args)
        dataset_train_burnin = build_dataset(image_set='burnin',
                                             label=True,
                                             args=args)
    ...
    if args.dataset_file == "coco":
        ...
        if args.distributed:
            if args.cache_mode:
                sampler_train_label = samplers.NodeDistributedSampler(
                    dataset_train_label)
                sampler_train_unlabel = samplers.NodeDistributedSampler(
                    dataset_train_unlabel)
                sampler_train_burnin = samplers.NodeDistributedSampler(
                    dataset_train_burnin)
                sampler_val = samplers.NodeDistributedSampler(dataset_val,
                                                              shuffle=False)
            else:
                sampler_train_label = samplers.DistributedSampler(
                    dataset_train_label)
                sampler_train_unlabel = samplers.DistributedSampler(
                    dataset_train_unlabel)
                sampler_train_burnin = samplers.DistributedSampler(
                    dataset_train_burnin)
                sampler_val = samplers.DistributedSampler(dataset_val,
                                                          shuffle=False)
        else:
            sampler_train_label = torch.utils.data.RandomSampler(
                dataset_train_label)
            sampler_train_unlabel = torch.utils.data.RandomSampler(
                dataset_train_unlabel)
            sampler_train_burnin = torch.utils.data.RandomSampler(
                dataset_train_burnin)
            sampler_val = torch.utils.data.SequentialSampler(dataset_val)
    ...
    if args.dataset_file == "coco":
        ...
        data_loader_train_label = DataLoader(
            dataset_train_label,
            batch_sampler=batch_sampler_train_label,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
            pin_memory=True)
        data_loader_train_unlabel = DataLoader(
            dataset_train_unlabel,
            batch_sampler=batch_sampler_train_unlabel,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
            pin_memory=True)
        data_loader_train_burnin = DataLoader(
            dataset_train_burnin,
            batch_sampler=batch_sampler_train_burnin,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
            pin_memory=True)
    ...
    param_dicts = [{
        ...
        "params": [
            p for n, p in model_without_ddp_student.named_parameters() if
    ...
    if args.sgd:
        ...
        optimizer_burnin = torch.optim.SGD(param_dicts,
                                           lr=2e-4,
                                           momentum=0.9,
                                           weight_decay=args.weight_decay)
    else:
        optimizer = torch.optim.AdamW(param_dicts,
                                      lr=args.lr,
                                      weight_decay=args.weight_decay)
        optimizer_burnin = torch.optim.AdamW(param_dicts,
                                             lr=2e-4,
                                             weight_decay=args.weight_decay)

    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop)
    lr_scheduler_burnin = torch.optim.lr_scheduler.StepLR(
        optimizer_burnin, args.lr_drop)
    ...
    if args.distributed:
        ...
        model_teacher = torch.nn.parallel.DistributedDataParallel(
            model_teacher, device_ids=[args.gpu])
    ...
    if args.frozen_weights is not None:
        ...
        model_without_ddp_student.detr.load_state_dict(
            checkpoint['model_student'])
        model_without_ddp_teacher.detr.load_state_dict(
    ...
    if args.resume:
        ...
        if 'model' in checkpoint.keys():  # means load burn in model
            ...
            TEACHER_LOADED = False
        elif 'model_student' in checkpoint.keys(
        ) and 'model_teacher' not in checkpoint.keys():
            missing_keys, unexpected_keys = model_without_ddp_student.load_state_dict(
    ...
    if args.eval:
        test_stats, coco_evaluator = evaluate(model_student, criterion,
                                              postprocessors, data_loader_val,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch < args.BURN_IN_STEP:
            train_stats = train_one_epoch_burnin(model_student,
                                                 criterion_burnin,
                                                 data_loader_train_burnin,
                                                 optimizer_burnin, device,
            ...
            if epoch >= args.BURN_IN_STEP:
                if not TEACHER_LOADED:
                    ...
                    if not OPTIMIZOR_LOADED:
                        ...
                        for pg, pg_old in zip(optimizer.param_groups,
                                              p_groups):
                            pg['lr'] = pg_old['lr']
                            pg['initial_lr'] = pg_old['initial_lr']
                        # print(optimizer.param_groups)
            ...
            train_stats = train_one_epoch_semi(
                model_student, model_teacher, criterion,
                data_loader_train_label, data_loader_train_unlabel, optimizer,
                device, epoch, args.clip_max_norm, args.CONFIDENCE_THRESHOLD,
                args.EMA_KEEP_RATE, args.PASSED_NUM, args.dataset_file,
            ...
            if args.output_dir:
                ...
                for checkpoint_path in checkpoint_paths:
                    ...
                    utils.save_on_master(
                        {
                            'model_student':
                            model_without_ddp_student.state_dict(),
                            'model_teacher':
                            model_without_ddp_teacher.state_dict(),
            ...
            if (epoch + 1) % args.eval_freq == 0:
                ...
                test_stats_student, coco_evaluator = evaluate(
                    model_student, criterion, postprocessors, data_loader_val,
                    base_ds, device, args.output_dir)

                test_stats_teacher, coco_evaluator = evaluate(
                    model_teacher, criterion, postprocessors, data_loader_val,
                    base_ds, device, args.output_dir)
                ...
                log_stats = {
                    ...
                    **{
                        f'test_stu_{k}': v
                        for k, v in test_stats_student.items()
                    },
                    **{
                        f'test_tea_{k}': v
                        for k, v in test_stats_teacher.items()
                    }, 'epoch': epoch,
                    'n_parameters': n_parameters
                }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'Omni-DETR training and evaluation script',
